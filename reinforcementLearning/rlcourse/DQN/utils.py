...
def plot_learning_curve(x, scores, epsilons, filename):
    ...
    movmean = np.empty(N)
    for t in range(N):
        movmean[t] = np.mean(scores[max(0, t-100):(t+1)])

    ax2.scatter(x,movmean, color="C1")
...
class RepeatActionAndMaxFrame(gym.Wrapper):
    def __init__(self, env=None, repeat=4, clip_rewards=False, num_ops=0, fire_first=False):
...
class PreprocessFrame(gym.ObservationWrapper):
    def __init__(self, env, shape):
        ...
        self.shape = (shape[2], shape[0], shape[1]) # image channels first
    ...
    def observation(self, obs):
        ...
        new_obs = np.array(resized_screen, dtype=np.uint8).reshape(self.shape) / 255.0
        return new_obs
